import { getApiUrl, getCliConfig } from '../util/cli-config';
...
export default async function whoami(_options, command: Command) {
  ...
  if (userAccessToken && !passedProfileOption) {
    ...
    console.log(
      'You are not authenticated. Either login with `truffle-cli login` or configure an org api key in ~/.truffle/config.json',
    );
  }
}
