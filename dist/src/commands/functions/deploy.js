...
export default async function deploy({ functionName, all } = {}) {
    ...
    const handleFunction = async (fn) => {
        ...
        const secrets = Reflect.get(pkgConfig.secrets, slug) || {};
        if (!Object.keys(secrets).length)
            console.log(chalk.yellow(`[commands::handleFunction] No secrets found for function ${slug}`));
        else
            console.log(chalk.gray(`[commands::handleFunction] including secrets: ${Object.keys(secrets).join(', ')}`));
