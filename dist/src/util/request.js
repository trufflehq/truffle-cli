...
export async function request({ query, variables, shouldUseGlobal = false }) {
    ...
    if (data?.errors?.length) {
        throw new Error(`Request error: ${JSON.stringify(data.errors[0].extensions.info ?? data.errors[0].message)}`);
    }
    return data;
}
export async function upload({ query, variables, bundle, shouldUseGlobal = false }) {
    const { apiUrl, secretKey } = shouldUseGlobal ? getGlobalConfig() : await getPackageConfig() || getGlobalConfig();
