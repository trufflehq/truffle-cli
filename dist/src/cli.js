...
program.addCommand(new Command('clone')
    ...
    const { default: clone } = await import('./commands/clone.js');
...
program.addCommand(new Command('dev')
...
program.addCommand(new Command('fork')
    .description('Fork an existing package.')
    .argument('<package-path>', 'The name of the package to fork.')
    .argument('<package-name>', 'The name of the new package.')
    .action(async (packagePath, packageName) => {
    const { default: fork } = await import('./commands/fork.js');
    await fork({ packagePath, toPackageSlug: packageName });
}));
program.addCommand(new Command('regenerate-api-key')
    .description('Request a new API Key.')
    .action(async () => {
    const { default: regeneratePackageApiKey } = await import('./commands/regenerate.js');
    await regeneratePackageApiKey();
}));
program.addCommand(new Command('install')
...
program.addCommand(new Command('pull')
    .description('Fetch the upstream package.')
    .action(async () => {
    const { default: pull } = await import('./commands/pull.js');
    await pull();
}));
program.addCommand(new Command('watch')
    .description('Watch for changes.')
    .action(async () => {
    const { watch } = await import('./commands/deploy.js');
    await watch();
}));
program.addCommand(new Command('ls')
    .description('List existing packages.')
    .action(async () => {
    const { default: list } = await import('./commands/list.js');
    await list();
}));
program.addCommand(new Command('functions')
...
program.addCommand(new Command('describe')
    ...
    .action(async (model) => {
    const { default: describe } = await import('./commands/describe.js');
